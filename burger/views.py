...
from burger.models import Burger, StripePayment, Payment, Order, Category
...
def home_page_view(request):
    categories = Category.objects.all()[1:4]
    last_three_burger = []
    for category in categories:
        last_three_burger.append(
            Burger.objects.filter(category=category).first()
        )
...
def add_cart(request, product_id):
    ...
    quantity = int(request.POST.get('quantity'))  # Default to 1 if quantity not provided
    if quantity > 0:
...
def process_payment(request):
    ...
    if request.method == 'POST':
        amount = total_amount
        payment_method = request.POST.get('payment_method')  # Assuming payment method is selected in the form
        payment = Payment.objects.create(client=client, amount=amount, payment_method=payment_method)
    ...
    return render(request, 'burger/payment.html', context)
